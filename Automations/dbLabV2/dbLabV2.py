...
# Desabilitar avisos de SSL (necessário para API do DB)
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

# Nota: Script adaptado para buscar dados do DB Toxicológico via API
...
def detectar_maquina():
    ...
    if username == 'washington.gouvea' or 'synvia' in hostname or 'washington' in hostname:
        base_dir = r'C:\Users\washington.gouvea\OneDrive - Synvia Group\Data Analysis\Churn PCLs\Automations\db'
    ...
    elif username == 'washi' or 'washi' in hostname:
        base_dir = r'D:\OneDrive - Synvia Group\Data Analysis\Churn PCLs\Automations\db'
        threads = 16  # Ryzen: pode usar mais por ser I/O bound
        machine = 'PC Casa (Ryzen 7 7500X)'
    # MÁQUINA NÃO RECONHECIDA - usar diretório atual
    else:
        # Usar diretório atual do script como base
        script_dir = os.path.dirname(os.path.abspath(__file__))
        base_dir = os.path.join(script_dir, 'arquivos')
        threads = 8  # Threads padrão conservador
        machine = f'Computador não reconhecido ({username}@{hostname})'
...
# Criar diretório se não existir (com tratamento de erro)
if not os.path.exists(BASE_DIR):
    try:
        os.makedirs(BASE_DIR, exist_ok=True)
        print(f"✓ Diretório criado: {BASE_DIR}")
    except (OSError, FileNotFoundError) as e:
        # Se falhar, usar diretório atual do script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        BASE_DIR = os.path.join(script_dir, 'arquivos')
        os.makedirs(BASE_DIR, exist_ok=True)
        print(f"⚠️  Não foi possível criar diretório original. Usando: {BASE_DIR}")
        logging.warning(f"Falha ao criar diretório original. Usando fallback: {BASE_DIR}")
...
class LabScraperV2:
    """Classe para fazer scraping dos dados de laboratórios do DB Toxicológico"""
    
    def __init__(self):
        # URLs base para as requisições do DB
        self.URL_ESTADOS = "https://compra.dbtoxicologico.com.br/venda/PostoColeta/Estados"
        self.URL_CIDADES = "https://compra.dbtoxicologico.com.br/venda/PostoColeta/Cidades?SiglaEstado={sigla_estado}"
        self.URL_LABORATORIOS = "https://compra.dbtoxicologico.com.br/venda/PostoColeta/ListarPostos"
        
        # Configurar sessão com TLS customizado (mesmo do db_robot.py)
        self.session = requests.Session()
        self.session.mount("https://", self._create_tls_adapter())
        
        self.checkpoint_file = os.path.join(BASE_DIR, "checkpoint_scraper.json")
        self.global_report_file = os.path.join(BASE_DIR, "relatorio_completo_laboratorios_db.xlsx")
        self.municipios = self.carregar_municipios_brasil()
    
    def _create_tls_adapter(self):
        """Cria adapter customizado para reduzir o nível de segurança do OpenSSL"""
        class TLSAdapter(HTTPAdapter):
            def init_poolmanager(self, connections, maxsize, block=False, **pool_kwargs):
                ctx = ssl.create_default_context()
                # Reduz o nível de segurança para aceitar chaves DH menores (não recomendado para produção)
                ctx.set_ciphers('DEFAULT:@SECLEVEL=1')
                # Desabilitar verificação de certificado como fallback
    ...
    def buscar_postos_multiplos_formatos(self, municipio: str, uf: str) -> List[Dict]:
        """
        Busca postos na API do DB Toxicológico por cidade e estado
        ...
        """
        try:
            postos = self.buscar_postos(municipio, uf)
            if postos:
                # Usar CNPJ como chave única para evitar duplicatas
                postos_unicos = {}
                for posto in postos:
                    cnpj = posto.get('Cnpj', '').strip()
                    if cnpj and cnpj not in postos_unicos:
                        postos_unicos[cnpj] = posto
                    elif not cnpj:
                        # Fallback para EntidadeId se não houver CNPJ
            ...
            logging.debug(f"Erro ao buscar postos para {municipio}-{uf}: {e}")
            return []
    ...
    def buscar_cidades(self, busca: str = "") -> List[Dict]:
        """
        Busca cidades disponíveis (compatibilidade - não usado no DB)
        
        Args:
            busca: Termo de busca para filtrar cidades (não usado)
            
        Returns:
            Lista vazia (método mantido para compatibilidade)
        """
        # Este método não é mais usado, mas mantido para compatibilidade
        return []
    
    def _mapear_dados_db_para_gralab(self, lab_db: Dict, municipio: str, uf: str) -> Dict:
        """
        Mapeia dados retornados pela API do DB Toxicológico para o formato Gralab
        
        Args:
            lab_db: Dicionário com dados do laboratório retornado pela API DB
            municipio: Nome do município usado na busca
            uf: Sigla do estado (UF)
            
        Returns:
            Dicionário com dados no formato Gralab
        """
        # Extrair valor do exame e formatar como serviço
        valor_exame = lab_db.get('ValorExame', '')
        servicos = []
        
        if valor_exame:
            try:
                valor_float = float(valor_exame)
                servicos.append(f"Exame Toxicológico - R$ {valor_float:.2f}")
            except (ValueError, TypeError):
                servicos.append(f"Exame Toxicológico - R$ {valor_exame}")
        
        # Construir endereço completo
        endereco_parts = []
        if lab_db.get('Endereco'):
            endereco_parts.append(lab_db.get('Endereco'))
        if lab_db.get('Numero'):
            endereco_parts.append(lab_db.get('Numero'))
        endereco_completo = ', '.join(endereco_parts) if endereco_parts else lab_db.get('EnderecoCompleto', '')
        
        # Usar IdEntidade como EntidadeId
        entidade_id = lab_db.get('IdEntidade')
        
        # Verificar se a API retorna coordenadas (testar diferentes nomes de campos possíveis)
        latitude = None
        longitude = None
        
        # Tentar diferentes variações de nomes de campos para coordenadas
        possiveis_lat = ['Latitude', 'latitude', 'Lat', 'lat', 'LatitudeGPS', 'LatitudeGPS']
        possiveis_lng = ['Longitude', 'longitude', 'Lng', 'lng', 'LongitudeGPS', 'LongitudeGPS']
        
        for campo_lat in possiveis_lat:
            if campo_lat in lab_db and lab_db[campo_lat]:
                try:
                    latitude = float(lab_db[campo_lat])
                    break
                except (ValueError, TypeError):
                    continue
        
        for campo_lng in possiveis_lng:
            if campo_lng in lab_db and lab_db[campo_lng]:
                try:
                    longitude = float(lab_db[campo_lng])
                    break
                except (ValueError, TypeError):
                    continue
        
        # Se não encontrou coordenadas, verificar se há um campo de coordenadas combinado
        if not latitude or not longitude:
            # Verificar campos como 'Coordenadas', 'GPS', etc.
            if 'Coordenadas' in lab_db and lab_db['Coordenadas']:
                try:
                    coords = str(lab_db['Coordenadas']).split(',')
                    if len(coords) >= 2:
        ...
        # Se não encontrou coordenadas na API, preencher com mensagem informativa
        if not latitude or not longitude:
            latitude = "Não disponível na API"
            longitude = "Não disponível na API"
        
        # Mapear campos do DB para formato Gralab
        lab_gralab = {
            'Nome': lab_db.get('NomeFantasia', lab_db.get('RazaoSocial', '')),
            'Cnpj': lab_db.get('CNPJ', '').strip(),
            'Telefone': lab_db.get('Telefone', ''),
            'Endereco': endereco_completo,
            'Bairro': lab_db.get('Bairro', ''),
            'Complemento': lab_db.get('Complemento', ''),
            'Cidade': lab_db.get('Cidade', municipio.upper()),
            'UF': lab_db.get('UF', uf),
            'Latitude': latitude,  # Extraído da API se disponível, senão mensagem informativa
            'Longitude': longitude,  # Extraído da API se disponível, senão mensagem informativa
            'HorarioFuncionamento': lab_db.get('Horario', lab_db.get('Horarios', '')),
            'UrlProduto': '',  # DB não fornece URL
            'EntidadeId': entidade_id,
            'Servicos': servicos,
            'cidade_busca': f"{municipio} - {uf}",
            'municipio': municipio,
            'uf_busca': uf,
            # Campos adicionais do DB para referência
            'RazaoSocial': lab_db.get('RazaoSocial', ''),
            'Email': lab_db.get('Email', ''),
            'CEP': lab_db.get('CEP', ''),
            'ValorExame': valor_exame
        }
        
        return lab_gralab
    
    def buscar_postos(self, municipio: str, uf: str) -> List[Dict]:
        """
        Busca postos/laboratórios em uma cidade específica usando API do DB Toxicológico
        Implementa paginação para garantir que todos os laboratórios sejam coletados
        ...
        Returns:
            Lista de dicionários com informações dos postos no formato Gralab
        """
        laboratorios_gralab = []
        start = 0
        length = 100  # Tamanho da página (mesmo padrão do db_robot.py, mas maior para eficiência)
        
        try:
            while True:
                # Preparar dados para POST request (mesmo formato do db_robot.py)
                form_data = {
                    'start': start,
                    'length': length,
                    'uf': uf.upper(),
                ...
                # Fazer requisição POST para API do DB (com verify=False como fallback adicional)
                response = self.session.post(self.URL_LABORATORIOS, data=form_data, timeout=30, verify=False)
                ...
                dados_pagina = resultado['data']
                
                # Se não há dados nesta página, parar
                if not dados_pagina or len(dados_pagina) == 0:
                    break
                
                # Processar cada laboratório retornado
                for idx, lab_db in enumerate(dados_pagina):
                    # Formatar valor do exame se existir
                    valor_exame = lab_db.get('ValorExame', '')
                    if valor_exame:
                        try:
                            valor_float = float(valor_exame)
                            lab_db['ValorExame'] = f"{valor_float:.2f}"
                        except (ValueError, TypeError):
                            pass
                    
                    # DEBUG: Log campos disponíveis no primeiro laboratório da primeira página (apenas uma vez)
                    if start == 0 and idx == 0 and not hasattr(self, '_debug_campos_logado'):
                        campos_disponiveis = list(lab_db.keys())
                        campos_coordenadas = [c for c in campos_disponiveis if any(coord in c.lower() for coord in ['lat', 'lng', 'coord', 'gps', 'geocod'])]
                        
                        logging.info(f"DEBUG: Analisando campos retornados pela API do DB...")
                        logging.info(f"DEBUG: Total de campos: {len(campos_disponiveis)}")
                        
                        if campos_coordenadas:
                            logging.info(f"DEBUG: ✓ Campos de coordenadas encontrados: {campos_coordenadas}")
                            for campo in campos_coordenadas:
                                valor = lab_db.get(campo)
                                logging.info(f"DEBUG:   {campo} = {valor} (tipo: {type(valor).__name__})")
                        else:
                            logging.info(f"DEBUG: ✗ Nenhum campo de coordenadas encontrado na API do DB")
                            logging.info(f"DEBUG: Campos disponíveis na API: {', '.join(sorted(campos_disponiveis))}")
                        
                        # Marcar como já logado para não repetir
                        self._debug_campos_logado = True
                    
                    # Mapear para formato Gralab
                    lab_gralab = self._mapear_dados_db_para_gralab(lab_db, municipio, uf)
                    laboratorios_gralab.append(lab_gralab)
                
                # Se retornou menos que o tamanho da página, chegamos ao fim
                if len(dados_pagina) < length:
                    break
            ...
            return laboratorios_gralab
                
        except Exception as e:
            logging.error(f"Erro ao buscar postos na cidade '{municipio}' ({uf}): {e}")
            return laboratorios_gralab  # Retornar o que já foi coletado mesmo em caso de erro
    ...
    def extrair_precos_servicos(servicos_str, debug=False):
        ...
        try:
            ...
            for servico in servicos:
                ...
                # DB Toxicológico: procurar por "TOXICOLÓGICO" ou "EXAME"
                if ('TOXICOLÓGICO' in servico_upper or 'EXAME' in servico_upper) and 'R$' in servico:
                    try:
                        preco = servico.split('R$')[1].strip().split()[0]
                        # DB geralmente tem apenas um tipo de exame, então podemos colocar em CNH como padrão
                        # ou criar uma lógica específica
                        if not precos['preco_cnh']:  # Se CNH não foi preenchido, usar este
                            precos['preco_cnh'] = preco
                        if not precos['preco_concurso']:  # Também pode ser usado para concurso
                            precos['preco_concurso'] = preco
                        if not precos['preco_clt']:  # E para CLT
                            precos['preco_clt'] = preco
                        if debug:
                            logging.debug(f"Preço Exame Toxicológico extraído: {preco}")
                    except Exception as e:
                        if debug:
                            logging.debug(f"Erro ao extrair preço Exame: {e}")
                
                elif 'CONCURSO' in servico_upper and 'R$' in servico:
                    try:
                        preco = servico.split('R$')[1].strip().split()[0]
                        precos['preco_concurso'] = preco
                        if debug:
    ...
    def obter_estados_cidades_db(self) -> List[Tuple[str, str]]:
        """
        Obtém lista de estados e cidades da API do DB Toxicológico
        
        Returns:
            Lista de tuplas (cidade, UF)
        """
        municipios_lista = []
        
        try:
            # Buscar estados
            logging.info("Buscando estados do DB...")
            response_estados = self.session.get(self.URL_ESTADOS, timeout=30, verify=False)
            response_estados.raise_for_status()
            estados = response_estados.json()
            
            if not estados:
                logging.error("Erro ao obter estados do DB. Usando lista estática como fallback.")
                return self.carregar_municipios_brasil()
            
            logging.info(f"Encontrados {len(estados)} estados no DB")
            
            # Para cada estado, buscar cidades
            for sigla_uf, nome_estado in estados.items():
                try:
                    url_cidades = self.URL_CIDADES.format(sigla_estado=sigla_uf)
                    response_cidades = self.session.get(url_cidades, timeout=30, verify=False)
                    response_cidades.raise_for_status()
                    cidades = response_cidades.json()
                    
                    if cidades:
                        for nome_cidade in cidades.keys():
                            municipios_lista.append((nome_cidade, sigla_uf))
                        logging.info(f"Estado {nome_estado} ({sigla_uf}): {len(cidades)} cidades")
                    
                    time.sleep(0.1)  # Pequeno delay para não sobrecarregar API
                except Exception as e:
                    logging.warning(f"Erro ao buscar cidades do estado {nome_estado} ({sigla_uf}): {e}")
                    continue
            
            logging.info(f"Total de {len(municipios_lista)} municípios obtidos do DB")
            return municipios_lista
            
        except Exception as e:
            logging.error(f"Erro ao obter estados/cidades do DB: {e}. Usando lista estática como fallback.")
            return self.carregar_municipios_brasil()
    ...
    def coletar_todos_postos(self, usar_lista_completa: bool = True, retomar: bool = True, max_workers: int = 16) -> pd.DataFrame:
        ...
        Busca estados e cidades dinamicamente da API do DB Toxicológico
        
        Args:
            usar_lista_completa: Se True, busca estados/cidades do DB. Se False, usa lista estática
        ...
        """
        # Carregar municípios do DB ou lista estática
        if usar_lista_completa:
            logging.info("Buscando estados e cidades da API do DB...")
            municipios = self.obter_estados_cidades_db()
            if not municipios:
                logging.warning("Não foi possível obter municípios do DB. Usando lista estática...")
        ...
        # Calcular quantos estados estão sendo processados
        estados_unicos = sorted(set(uf for _, uf in municipios))
        municipios_por_estado = {}
        ...
        logging.info(f"Estados a processar: {len(estados_unicos)} estados ({', '.join(estados_unicos)})")
        print(f"\n{'='*60}")
        print(f"Iniciando coleta em {len(municipios)} municípios")
        print(f"📍 Processando TODOS os estados brasileiros: {len(estados_unicos)} estados")
        print(f"🧵 Usando {max_workers} threads simultâneas")
        print(f"{'='*60}\n")
        sys.stdout.flush()
        
        # Estruturas compartilhadas thread-safe
        postos_unicos = {}  # Dicionário para controlar duplicatas por CNPJ
        todos_postos = []
            ...
            try:
                ...
                if postos:
                    for posto in postos:
                        ...
                        with lock_postos:
                            # Evita duplicatas usando CNPJ como chave única (mais confiável)
                            if cnpj and cnpj not in postos_unicos:
                                postos_unicos[cnpj] = posto
                                todos_postos.append(posto)
                                novos_labs += 1
                            elif not cnpj:
                                # Fallback para EntidadeId apenas se não houver CNPJ
                                entidade_id = posto.get('EntidadeId')
                                if entidade_id and entidade_id not in postos_unicos:
                                    postos_unicos[entidade_id] = posto
                                    todos_postos.append(posto)
                ...
                for future in as_completed(future_to_municipio):
                    try:
                        ...
                        if primeiro_resultado:
                            print(f"✓ Primeiro resultado recebido (processando todos os estados): {resultado['municipio']}-{resultado['uf']}")
                            sys.stdout.flush()
        ...
        for posto in todos_postos:
            uf = posto.get('Uf', 'N/A')
            labs_por_estado[uf] = labs_por_estado.get(uf, 0) + 1
        
        logging.info(f"\nRESUMO DA COLETA:")
        logging.info(f"  - Estados processados: {len(estados_unicos)} estados")
        ...
        # Log de laboratórios por estado (apenas estados com labs)
        if labs_por_estado:
            logging.info(f"\n  Laboratórios por estado:")
            for uf in sorted(labs_por_estado.keys()):
                logging.info(f"    {uf}: {labs_por_estado[uf]} laboratórios")
        ...
        if todos_postos:
            ...
            # REMOVER DUPLICATAS POR CNPJ (garantia final)
            logging.info("Removendo duplicatas por CNPJ...")
            duplicatas_antes = len(df)
            
            # Remover duplicatas mantendo a primeira ocorrência
            # Primeiro, normalizar CNPJs (remover espaços, converter para string)
            df['Cnpj'] = df['Cnpj'].astype(str).str.strip()
            
            # Remover linhas onde CNPJ está vazio ou é 'nan'
            df = df[df['Cnpj'].notna() & (df['Cnpj'] != '') & (df['Cnpj'] != 'nan')]
            
            # Remover duplicatas por CNPJ, mantendo a primeira ocorrência
            df = df.drop_duplicates(subset=['Cnpj'], keep='first')
            
            duplicatas_removidas = duplicatas_antes - len(df)
            if duplicatas_removidas > 0:
                logging.info(f"✓ Removidas {duplicatas_removidas} duplicatas por CNPJ")
                print(f"✓ Removidas {duplicatas_removidas} duplicatas por CNPJ")
    ...
    def marcar_pipeline_completo(self, data_str: str):
        """
        Adiciona marcação ao arquivo indicando que o pipeline foi completamente executado
        Atualiza o arquivo existente em vez de criar um novo para cada data
        Se a data já existir, atualiza a entrada existente
        
        Args:
            data_str: Data no formato 'YYYY-MM-DD'
        """
        flag_file = os.path.join(BASE_DIR, ".pipeline_completo.flag")
        
        # Se o arquivo existe, verifica se a data já está lá e atualiza
        if os.path.exists(flag_file):
            try:
                with open(flag_file, 'r', encoding='utf-8') as f:
                    lines = f.readlines()
                
                # Remove a linha da data se já existir
                filtered_lines = [line for line in lines if not line.startswith(f"{data_str}:")]
                
                # Adiciona a nova entrada
                filtered_lines.append(f"{data_str}: Pipeline executado em {datetime.now().isoformat()}\n")
                
                # Reescreve o arquivo
                with open(flag_file, 'w', encoding='utf-8') as f:
                    f.writelines(filtered_lines)
            except Exception as e:
                logging.warning(f"Erro ao atualizar flag para {data_str}: {e}")
                # Em caso de erro, tenta adicionar no final
                with open(flag_file, 'a', encoding='utf-8') as f:
                    f.write(f"{data_str}: Pipeline executado em {datetime.now().isoformat()}\n")
        else:
            # Se o arquivo não existe, cria com a primeira entrada
            with open(flag_file, 'w', encoding='utf-8') as f:
                f.write(f"{data_str}: Pipeline executado em {datetime.now().isoformat()}\n")
    ...
    def verificar_pipeline_completo(self, data_str: str) -> bool:
        ...
        Verifica se a data específica já está no arquivo único de flags
        ...
        # Verifica se a data específica já está no arquivo
        try:
            with open(flag_file, 'r', encoding='utf-8') as f:
                for line in f:
                    if line.startswith(f"{data_str}:"):
                        return True
        except Exception:
            return False
    ...
    def remover_entrada_pipeline_completo(self, data_str: str):
        """
        Remove a entrada de uma data específica do arquivo de flags
        ...
        try:
            # Lê todas as linhas exceto a da data especificada
            with open(flag_file, 'r', encoding='utf-8') as f:
                lines = f.readlines()
            
            # Filtra removendo a linha da data especificada
            filtered_lines = [line for line in lines if not line.startswith(f"{data_str}:")]
            ...
            logging.warning(f"Erro ao remover entrada do flag para {data_str}: {e}")
    ...
    def run_pipeline(self, max_workers: int = 16):
        ...
        if self.verificar_pipeline_completo(today):
            ...
            print(f"\n💡 Se deseja reprocessar, use: python dbLabV2.py --force")
            print(f"   Ou delete manualmente a entrada de {today} no arquivo: .pipeline_completo.flag")
            print(f"{'='*70}\n")
...
def main():
    ...
    # Se --force foi usado, remover entrada de pipeline completo para a data atual
    if force_reprocess:
        today = datetime.now().strftime('%Y-%m-%d')
        if scraper.verificar_pipeline_completo(today):
            scraper.remover_entrada_pipeline_completo(today)
            print(f"✓ Entrada de pipeline completo removida para {today}. Reprocessando...\n")
            logging.info(f"Entrada de pipeline completo removida para {today}. Forçando reprocessamento.")
