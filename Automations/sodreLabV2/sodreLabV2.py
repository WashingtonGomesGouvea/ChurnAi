...
# Nota: Não é mais necessário carregar variáveis de ambiente, pois a API do Sodre é pública
...
def detectar_maquina():
    ...
    if username == 'washington.gouvea' or 'synvia' in hostname or 'washington' in hostname:
        base_dir = r'C:\Users\washington.gouvea\OneDrive - Synvia Group\Data Analysis\Churn PCLs\Automations\sodre'
    ...
    # PC CASA (Ryzen 7 7500X - 6 cores / 12 threads)
        ...
        base_dir = r'D:\OneDrive - Synvia Group\Data Analysis\Churn PCLs\Automations\sodre'
...
class LabScraperV2:
    """Classe para fazer scraping dos dados de laboratórios do Sodre"""
    
    def __init__(self):
        self.base_url = "https://li-sodretox-af-cidades.azurewebsites.net/api"
        self.checkpoint_file = os.path.join(BASE_DIR, "checkpoint_scraper.json")
        self.global_report_file = os.path.join(BASE_DIR, "relatorio_completo_laboratorios_sodre.xlsx")
    ...
    def buscar_postos_multiplos_formatos(self, municipio: str, uf: str) -> List[Dict]:
        """
        Busca postos testando múltiplos formatos de nome de cidade para API do Sodre
        ...
        """
        # Formatos para testar na API do Sodre
        formatos = [
            f"{municipio.upper()} - {uf}",  # Ex: "CAMPINAS - SP" (formato preferido)
            municipio.upper(),  # Ex: "CAMPINAS" (sem UF)
        ]
        ...
        for formato in formatos:
            try:
                ...
                if postos:
                    for posto in postos:
                        # Usar CNPJ como chave única (mais confiável que EntidadeId)
                        cnpj = posto.get('Cnpj')
                        if cnpj and cnpj not in postos_unicos:
                            postos_unicos[cnpj] = posto
                        # Também verificar por EntidadeId como fallback
                        elif not cnpj:
                ...
                logging.debug(f"Erro ao buscar postos com formato '{formato}': {e}")
                continue
    ...
    def buscar_cidades(self, busca: str = "") -> List[Dict]:
        """
        Busca cidades disponíveis (compatibilidade - não usado no Sodre)
    ...
    def _mapear_dados_sodre_para_gralab(self, lab_sodre: Dict, municipio: str, uf: str) -> Dict:
        """
        Mapeia dados retornados pela API do Sodre para o formato Gralab
        
        Args:
            lab_sodre: Dicionário com dados do laboratório retornado pela API Sodre
        ...
        """
        # Normalizar campo servicos (pode vir como string ou lista)
        servicos = lab_sodre.get('servicos', [])
        if isinstance(servicos, str):
            try:
                import ast
                servicos = ast.literal_eval(servicos)
            except:
                servicos = [servicos] if servicos else []
        elif not isinstance(servicos, list):
            servicos = []
        
        # Gerar EntidadeId baseado em hash do CNPJ
        cnpj = lab_sodre.get('cnpj', '')
        entidade_id = hash(f"{cnpj}{municipio}{uf}") % 1000000 if cnpj else None
        
        # Mapear campos do Sodre para formato Gralab
        lab_gralab = {
            'Nome': lab_sodre.get('nome', ''),
            'Cnpj': cnpj,
            'Telefone': lab_sodre.get('telefone', ''),
            'Endereco': lab_sodre.get('endereco', ''),
            'Bairro': lab_sodre.get('bairro', ''),
            'Complemento': lab_sodre.get('complemento'),
            'Cidade': lab_sodre.get('cidade', municipio.upper()),
            'UF': lab_sodre.get('uf', uf),
            'Latitude': lab_sodre.get('latitude'),
            'Longitude': lab_sodre.get('longitude'),
            'HorarioFuncionamento': lab_sodre.get('horarioFuncionamento', ''),
            'UrlProduto': lab_sodre.get('urlProduto', ''),
    ...
    def buscar_postos(self, cidade: str) -> List[Dict]:
        """
        Busca postos/laboratórios em uma cidade específica usando API do Sodre
        Busca tanto para CNH quanto para Concurso Público e combina os resultados
        
        Args:
            cidade: Nome da cidade (ex: "CAMPINAS - SP" ou "Campinas - SP")
            
        Returns:
            Lista de dicionários com informações dos postos no formato Gralab
        """
        # Extrair UF e nome da cidade da string
        cidade_upper = cidade.upper()
        uf_match = re.search(r'\b([A-Z]{2})\b', cidade_upper)
        uf = uf_match.group(1) if uf_match else None
        
        # Remover UF e normalizar nome da cidade
        cidade_nome = re.sub(r'\s*-\s*[A-Z]{2}\s*$', '', cidade_upper).strip()
        
        # Formatar cidade para URL (formato: "Cidade%20-%20UF")
        if uf:
            cidade_formatada = f"{cidade_nome}%20-%20{uf}"
        else:
            cidade_formatada = cidade_nome.replace(' ', '%20')
        
        # Buscar para todas as finalidades (CNH, Concurso Público e CLT)
        finalidades = ['CNH', 'Concurso Público', 'Empregado CLT']
        todos_laboratorios = {}  # Usar CNPJ como chave para combinar
        
        for finalidade in finalidades:
            # Construir URL da API do Sodre
            finalidade_encoded = finalidade.replace(' ', '%20')
            url = f"{self.base_url}/BuscarPostos?cidade={cidade_formatada}&finalidade={finalidade_encoded}"
            ...
            try:
                ...
                # Extrair lista de laboratórios do campo 'objeto'
                laboratorios_sodre = resultado.get('objeto', [])
                
                # Processar cada laboratório
                for lab_sodre in laboratorios_sodre:
                    cnpj = lab_sodre.get('cnpj', '')
                    
                    if cnpj:
                        # Se já existe, combinar serviços
                        if cnpj in todos_laboratorios:
                            # Combinar serviços das duas finalidades
                            servicos_existentes = todos_laboratorios[cnpj].get('Servicos', [])
                            servicos_novos = lab_sodre.get('servicos', [])
                            
                            # Normalizar servicos_novos
                            if isinstance(servicos_novos, str):
                                try:
                                    import ast
                                    servicos_novos = ast.literal_eval(servicos_novos)
                                except:
                                    servicos_novos = [servicos_novos] if servicos_novos else []
                            elif not isinstance(servicos_novos, list):
                                servicos_novos = []
                            
                            # Combinar listas sem duplicatas
                            servicos_combinados = list(servicos_existentes)
                            for servico in servicos_novos:
                                if servico not in servicos_combinados:
                                    servicos_combinados.append(servico)
                            
                            todos_laboratorios[cnpj]['Servicos'] = servicos_combinados
                        else:
                            # Primeira vez que vemos este laboratório
                            lab_gralab = self._mapear_dados_sodre_para_gralab(lab_sodre, cidade_nome, uf or '')
                            todos_laboratorios[cnpj] = lab_gralab
                    else:
                        # Se não tem CNPJ, adicionar mesmo assim (pode ter EntidadeId)
                        lab_gralab = self._mapear_dados_sodre_para_gralab(lab_sodre, cidade_nome, uf or '')
                        entidade_id = lab_gralab.get('EntidadeId')
                        if entidade_id:
                            todos_laboratorios[f"NO_CNPJ_{entidade_id}"] = lab_gralab
                        else:
                            # Último recurso: usar hash do nome
                            nome_hash = hash(lab_gralab.get('Nome', '')) % 1000000
                            todos_laboratorios[f"NO_CNPJ_{nome_hash}"] = lab_gralab
                
            except Exception as e:
                logging.error(f"Erro ao buscar postos na cidade '{cidade}' para finalidade '{finalidade}': {e}")
                continue
        
        # Retornar lista de laboratórios combinados
        return list(todos_laboratorios.values())
    ...
    def run_pipeline(self, max_workers: int = 16):
        ...
        if self.verificar_pipeline_completo(today):
            ...
            print(f"\n💡 Se deseja reprocessar, use: python sodreLabV2.py --force")
